...
describe('Tree.map()', () => {
    it('should return an array result even if the single node is passed as an input', () => {
        ...
        let mapped = map(sourceNode, node => ({
            elementId: node.id,
            title: node.name
        }));
    ...
    it('should work for a simple array like tree', () => {
        ...
        let mappedTree = map(sourceTree, node => ({
            elementId: node.id,
            title: node.name,
            selected: true
        }));
    ...
    it('should work for a complex tree with nested nodes', () => {
        ...
        let mappedTree = map(tree1, ({id, name}) => ({nodeId: id, name}));
    ...
    it('should maintain the index correctly for nested nodes', () => {
        ...
        let mappedTree = map(tree1, (node, index) => ({index}));
